...
import com.greemoid.ithelps.domain.models.MoodType

class ChoiceMoodAdapter(
    private val dataSource: List<MoodType>
) : RecyclerView.Adapter<ChoiceMoodAdapter.ChoiceMoodViewHolder>() {
    inner class ChoiceMoodViewHolder(val binding: MoodItemsLayoutBinding) : RecyclerView.ViewHolder(binding.root)

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ChoiceMoodViewHolder {
        val binding = MoodItemsLayoutBinding.inflate(LayoutInflater.from(parent.context), parent, false)
